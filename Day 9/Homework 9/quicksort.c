 ...
 * quicksort.c
 *
 * this program uses quick sort to sort arrays.
...
/*
 * recursively sort part of the array using quick sort.
 * the middle value is used as the pivot.
 */
void quicksortrecursive(int a[], int left, int right) {
     ...
     * and larger values to the right side.
     */
    while (i <= j) {
        while (a[i] < pivot) {
            i++;
    ...
    /* recursively sort the two remaining sections */
    if (left < j) {
